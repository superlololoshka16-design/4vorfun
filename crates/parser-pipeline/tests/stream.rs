...
fn container_with_id_stays_in_tree() {
    let html = r#"<html><body><div id="app"><p>inner</p></div><div><p>junk</p></div></body></html>"#;
    let mut p = StreamPipeline::new(Default::default());
    p.push(html.as_bytes()).unwrap();
    let page = p.finish().unwrap();
    let dom = &page.dom;
...
<div>plain-no-marker-text</div>
    ...
    assert!(t.len() <= 128, "marker container text truncated, got {}", t.len());
...
fn dom_tree_structure_and_text() {
    ...
    assert_eq!(dom.attr(div, parser_pipeline::ATTR_NAMES["id"]), Some("app"));
    assert_eq!(dom.attr(div, parser_pipeline::ATTR_NAMES["class"]), Some("root"));
...
fn b64_field_decodes_and_rejects_garbage() {
    ...
    let n = parser_pipeline::decode_b64_field(b"  U2lsbyByb2Nrcw== ", &mut out).unwrap();
    assert_eq!(&out[..n], b"Silo rocks");
    out.clear();
    assert!(parser_pipeline::decode_b64_field(b"!!!not-base64!!!", &mut out).is_err());
}
