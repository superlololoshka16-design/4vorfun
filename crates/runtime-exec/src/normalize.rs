...
impl<'a, 's> Traverse<'a, &'s mut NormCtx<'s>> for NormPass {
    ...
    fn exit_expression(
        ...
        let (is_str, is_num) = match node {
            Expression::StringLiteral(lit) => (!lit.lone_surrogates, false),
            Expression::NumericLiteral(lit) => (false, lit.value.is_finite()),
            _ => (false, false),
        };
        if !is_str && !is_num {
            return;
        ...
        let idx = st.args.len();
        let lit = if is_str {
            let Expression::StringLiteral(sl) = node else {
                return;
            };
            Lit::Str(CompactString::new(sl.value.as_str()))
        } else {
            let Expression::NumericLiteral(nl) = node else {
                return;
            };
            Lit::Num(nl.value)
        };
        st.args.push(lit);
        let object = Expression::new_identifier(SPAN, "__silo_args", ctx);
        let index =
...
fn canon_ident<'a, A: oxc_allocator::GetAllocator<'a>>(idx: u32, alloc: &A) -> Ident<'a> {
    let mut buf: SmallVec<[u8; 12]> = SmallVec::new();
    let _ = write!(&mut buf, "v{idx}");
    Ident::from_str_in(
        std::str::from_utf8(&buf).expect("canonical name ascii"),
        alloc,
    )
}
...
pub fn normalize(script: &[u8]) -> Result<Normalized, ()> {
    ...
    OXC_ALLOC.with(|cell| {
        ...
        if n > 0 {
            ...
            let taken = std::mem::replace(last_slot, Statement::new_return_statement(SPAN, None, &ast));
